 ...
 * 备注：使用本地云检索、周边云检索、矩形云检索、详情云检索时需要申请“服务端”的ak，替换云检索的ak参数;
 *      使用云反地理编码检索时，不需要申请“服务端”的ak，需要在http://lbsyun.baidu.com/apiconsole/key找到app的ak，点击设置，启用“云逆地理编码API”服务。
...
//发起云RGC检索
- (IBAction)onClickCloudRgcSearch:(id)sender {
...
- (void)onGetCloudReverseGeoCodeResult:(BMKCloudReverseGeoCodeResult *)cloudRGCResult searchType:(BMKCloudSearchType)type errorCode:(NSInteger)errorCode {
    ...
    if (errorCode == BMKErrorOk) {
        ...
        //如果有自己的poi数据，则使用自己的poi数据添加标注，否则使用百度地图的poi数据添加标注
        if (cloudRGCResult.customPoiList && cloudRGCResult.customPoiList.count > 0) {
            //使用自己的poi数据添加标注
            for (BMKCloudPOIInfo *poiInfo in cloudRGCResult.poiList) {
                BMKPointAnnotation *annotation = [[BMKPointAnnotation alloc] init];
                annotation.coordinate = CLLocationCoordinate2DMake(poiInfo.latitude, poiInfo.longitude);
                annotation.title = poiInfo.title;
                [annotations addObject:annotation];
            }
        } else {
            //使用百度地图的poi数据添加标注
            for (BMKCloudMapPOIInfo *poiInfo in cloudRGCResult.poiList) {
                BMKPointAnnotation *annotation = [[BMKPointAnnotation alloc] init];
        ...
        [_mapView showAnnotations:annotations animated:NO];
    }
}
