...
- (void)initTheInterface
{
    ...
    self.mainPopoverListView.allowsMultipleSelection = YES;
    [self addSubview:self.mainPopoverListView];
    
    self.doneButton = [UIButton buttonWithType:UIButtonTypeCustom];
    self.doneButton.frame = CGRectMake(0, self.bounds.size.height-40.0f, xWidth, 40.0f);
...
{
     [[NSNotificationCenter defaultCenter] postNotificationName:@"SelectRoadType" object:nil userInfo:nil];
    [self dismiss];
}
...
- (void)popoverDeselectRowAtIndexPath:(NSIndexPath *)indexPath animated:(BOOL)animated
{
    return [self.mainPopoverListView deselectRowAtIndexPath:indexPath animated:YES];
}
